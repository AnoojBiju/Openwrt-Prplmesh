...
# Temporary workaround for mbo issue
ubus-cli WiFi.AccessPoint.*.MBOEnable=0
...
# Enable Wi-Fi radios
# Disable 6Ghz radio until it's supported
...
ubus call "WiFi.Radio" _set '{ "rel_path": ".[OperatingFrequencyBand == \"6GHz\"].", "parameters": { "Enable": "false" } }'
...
SERVER_CMD="sleep 20 && sh /etc/init.d/ssh-server stop|| true && dropbear -F -T 10 -p192.168.250.130:22 &"
