...
tlvVsChannelScanResult::tlvVsChannelScanResult(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
tlvVsChannelScanResult::tlvVsChannelScanResult(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
tlvVsChannelScanResult::~tlvVsChannelScanResult() {
}
sMacAddr& tlvVsChannelScanResult::radio_uid() {
    return (sMacAddr&)(*m_radio_uid);
}

uint8_t& tlvVsChannelScanResult::operating_class() {
    return (uint8_t&)(*m_operating_class);
}

uint8_t& tlvVsChannelScanResult::channel() {
    return (uint8_t&)(*m_channel);
}

uint8_t& tlvVsChannelScanResult::scan_results_list_length() {
    return (uint8_t&)(*m_scan_results_list_length);
}

std::tuple<bool, sChannelScanResults&> tlvVsChannelScanResult::scan_results_list(size_t idx) {
    bool ret_success = ( (m_scan_results_list_idx__ > 0) && (m_scan_results_list_idx__ > idx) );
    size_t ret_idx = ret_success ? idx : 0;
    if (!ret_success) {
        TLVF_LOG(ERROR) << "Requested index is greater than the number of available entries";
    }
    return std::forward_as_tuple(ret_success, m_scan_results_list[ret_idx]);
}

bool tlvVsChannelScanResult::alloc_scan_results_list(size_t count) {
    if (m_lock_order_counter__ > 0) {;
        TLVF_LOG(ERROR) << "Out of order allocation for variable length list scan_results_list, abort!";
        return false;
    }
    ...
    m_lock_order_counter__ = 0;
    uint8_t *src = (uint8_t *)&m_scan_results_list[*m_scan_results_list_length];
    uint8_t *dst = src + len;
    if (!m_parse__) {
        size_t move_length = getBuffRemainingBytes(src) - len;
        std::copy_n(src, move_length, dst);
    }
    m_scan_results_list_idx__ += count;
    *m_scan_results_list_length += count;
    if (!buffPtrIncrementSafe(len)) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << len << ") Failed!";
        return false;
    }
    if (!m_parse__) { 
        for (size_t i = m_scan_results_list_idx__ - count; i < m_scan_results_list_idx__; i++) { m_scan_results_list[i].struct_init(); }
    }
    return true;
}

void tlvVsChannelScanResult::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_1905_VS), reinterpret_cast<uint8_t*>(m_action_op));
    m_radio_uid->struct_swap();
    for (size_t i = 0; i < m_scan_results_list_idx__; i++){
        m_scan_results_list[i].struct_swap();
    }
}

bool tlvVsChannelScanResult::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t tlvVsChannelScanResult::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // radio_uid
    class_size += sizeof(uint8_t); // operating_class
    class_size += sizeof(uint8_t); // channel
    class_size += sizeof(uint8_t); // scan_results_list_length
    return class_size;
}

bool tlvVsChannelScanResult::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    ...
    m_scan_results_list_length = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!m_parse__) *m_scan_results_list_length = 0;
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
    m_scan_results_list = reinterpret_cast<sChannelScanResults*>(m_buff_ptr__);
    uint8_t scan_results_list_length = *m_scan_results_list_length;
    m_scan_results_list_idx__ = scan_results_list_length;
    if (!buffPtrIncrementSafe(sizeof(sChannelScanResults) * (scan_results_list_length))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(sChannelScanResults) * (scan_results_list_length) << ") Failed!";
        return false;
    }
    if (m_parse__) { class_swap(); }
    return true;
