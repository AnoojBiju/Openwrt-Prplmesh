...
cACTION_CLI_TEST_CONF::cACTION_CLI_TEST_CONF(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_CLI_TEST_CONF::cACTION_CLI_TEST_CONF(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_CLI_TEST_CONF::~cACTION_CLI_TEST_CONF() {
}
uint8_t& cACTION_CLI_TEST_CONF::value() {
    return (uint8_t&)(*m_value);
}

void cACTION_CLI_TEST_CONF::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_CLI), reinterpret_cast<uint8_t*>(m_action_op));
}

bool cACTION_CLI_TEST_CONF::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_CLI_TEST_CONF::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_CLI_TEST_CONF::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
