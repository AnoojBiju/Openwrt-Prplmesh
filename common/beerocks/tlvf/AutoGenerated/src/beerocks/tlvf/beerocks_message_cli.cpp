...
cACTION_CLI_TEST_CONFIG::cACTION_CLI_TEST_CONFIG(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_CLI_TEST_CONFIG::cACTION_CLI_TEST_CONFIG(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_CLI_TEST_CONFIG::~cACTION_CLI_TEST_CONFIG() {
}
uint8_t& cACTION_CLI_TEST_CONFIG::value() {
    return (uint8_t&)(*m_value);
}

void cACTION_CLI_TEST_CONFIG::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_CLI), reinterpret_cast<uint8_t*>(m_action_op));
}

bool cACTION_CLI_TEST_CONFIG::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_CLI_TEST_CONFIG::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(uint8_t); // value
    return class_size;
}

bool cACTION_CLI_TEST_CONFIG::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    }
    m_value = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
