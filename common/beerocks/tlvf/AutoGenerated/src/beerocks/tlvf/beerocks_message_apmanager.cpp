...
cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS::cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS::cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS::~cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS() {
}
sSpatialReuseParams& cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS::sr_params() {
    return (sSpatialReuseParams&)(*m_sr_params);
}

void cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
    m_sr_params->struct_swap();
}

bool cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sSpatialReuseParams); // sr_params
    return class_size;
...
bool cACTION_APMANAGER_SET_SPATIAL_REUSE_PARAMS::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
...
cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS::cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS::cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS::~cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS() {
}
void cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
}

bool cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS::get_initial_size()
{
    size_t class_size = 0;
    return class_size;
}

bool cACTION_APMANAGER_GET_SPATIAL_REUSE_PARAMS::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
