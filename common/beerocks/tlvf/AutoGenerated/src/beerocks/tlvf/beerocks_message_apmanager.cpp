...
cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST::cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST::cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST::~cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST() {
}
sSpatialReuseRequest& cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST::params() {
    return (sSpatialReuseRequest&)(*m_params);
}

void cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
    m_params->struct_swap();
}

bool cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST::get_initial_size()
{
    size_t class_size = 0;
...
bool cACTION_APMANAGER_HOSTAP_SPATIAL_REUSE_REQUEST::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
