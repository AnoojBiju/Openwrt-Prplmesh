...
cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::~cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY() {
}
uint8_t& cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::nw_info() {
    return (uint8_t&)(*m_nw_info);
}

sMacAddr& cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::sta_mac() {
    return (sMacAddr&)(*m_sta_mac);
}

void cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
    m_sta_mac->struct_swap();
}

bool cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(uint8_t); // nw_info
    class_size += sizeof(sMacAddr); // sta_mac
    return class_size;
}

bool cACTION_APMANAGER_HOSTAP_STA_INFO_QUERY::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    }
    m_nw_info = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
...
cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY::cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY::cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY::~cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY() {
}
sStaDeviceInfo& cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY::params() {
    return (sStaDeviceInfo&)(*m_params);
}

void cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_APMANAGER), reinterpret_cast<uint8_t*>(m_action_op));
    m_params->struct_swap();
}

bool cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sStaDeviceInfo); // params
    return class_size;
}

bool cACTION_APMANAGER_HOSTAP_STA_INFO_REPLY::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    }
    m_params = reinterpret_cast<sStaDeviceInfo*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(sStaDeviceInfo))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(sStaDeviceInfo) << ") Failed!";
        return false;
    }
    if (!m_parse__) { m_params->struct_init(); }
    if (m_parse__) { class_swap(); }
