...
cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC::cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC::cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC::~cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC() {
}
sApChannelSwitch& cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC::cs_params() {
    return (sApChannelSwitch&)(*m_cs_params);
}

void cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BACKHAUL), reinterpret_cast<uint8_t*>(m_action_op));
    m_cs_params->struct_swap();
}

bool cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sApChannelSwitch); // cs_params
    return class_size;
...
bool cACTION_BACKHAUL_HOSTAP_CANCEL_ACTIVE_CAC::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
