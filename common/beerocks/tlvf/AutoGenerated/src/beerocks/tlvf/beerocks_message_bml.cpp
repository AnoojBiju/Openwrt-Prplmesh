...
cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::~cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE() {
}
sMacAddr& cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::al_mac() {
    return (sMacAddr&)(*m_al_mac);
}

uint32_t& cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::rule_id() {
    return (uint32_t&)(*m_rule_id);
}

uint8_t& cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::add_remove() {
    return (uint8_t&)(*m_add_remove);
}

uint8_t& cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::precedence() {
    return (uint8_t&)(*m_precedence);
}

uint8_t& cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::output() {
    return (uint8_t&)(*m_output);
}

uint8_t& cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::always_match() {
    return (uint8_t&)(*m_always_match);
}

void cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::class_swap()
{
    tlvf_swap(8*sizeof(eActionOp_BML), reinterpret_cast<uint8_t*>(m_action_op));
    m_al_mac->struct_swap();
...
bool cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::finalize()
{
    if (m_parse__) {
        TLVF_LOG(DEBUG) << "finalize() called but m_parse__ is set";
        return true;
...
size_t cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::get_initial_size()
{
    size_t class_size = 0;
    class_size += sizeof(sMacAddr); // al_mac
    class_size += sizeof(uint32_t); // rule_id
    class_size += sizeof(uint8_t); // add_remove
    class_size += sizeof(uint8_t); // precedence
    class_size += sizeof(uint8_t); // output
    class_size += sizeof(uint8_t); // always_match
    return class_size;
}

bool cACTION_BML_TRIGGER_SERVICE_PRIORITIZATION_RULE::init()
{
    if (getBuffRemainingBytes() < get_initial_size()) {
        TLVF_LOG(ERROR) << "Not enough available space on buffer. Class init failed";
        return false;
    ...
    m_add_remove = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
    ...
    m_always_match = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
