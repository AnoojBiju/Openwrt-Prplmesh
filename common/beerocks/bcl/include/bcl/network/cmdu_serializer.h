 ...
 * During testing, we can serialize any CMDU to a buffer ready to be sent, but without actually
 * sending it. This way it is possible to test the serializer with any CMDU but without the
...
public:
     ...
     * @param[out] srct_mac Source MAC address.
     * @param[out] cmdu_rx serialized CMDU message.
     ...
     * does not fit into output bufffer).
     */
