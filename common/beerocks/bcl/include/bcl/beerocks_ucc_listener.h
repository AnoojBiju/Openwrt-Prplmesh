...
// Error string included in WFA-CA reply when no handler has been set by the application to handle
// received command.
constexpr auto unhandled_command_error_string = "No handler for command was set";
...
public:
     ...
     * asynchronous second reply to the command must be sent through this connection.
    ...
    using DevResetDefaultHandler =
        std::function<bool(int fd, const std::unordered_map<std::string, std::string> &params,
     ...
     * 
     * This method must be invoked when the processing of an asynchronous WFA-CA command is 
     * complete, in order to send the second reply to UCC client. 
     * 
     * Currently, there is only one asynchronous command which is "dev_reset_default".
     * 
     ...
     * @return true on success and false otherwise.
     */
    bool send_async_reply(int fd, const std::string &err_string = std::string());
                                  ...
                                  std::string &err_string) const
    {
        ...
        return m_handlers.on_dev_reset_default(fd, params, err_string);
    }
