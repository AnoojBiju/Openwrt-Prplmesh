...
*/
static bool assign_auth_encr_parameters(prplmesh::hostapd::Configuration &conf, std::string &vap_id,
    ...
    const std::string &backhaul_wps_ssid, const std::string &backhaul_wps_passphrase)
{
    ...
    for (const auto &bss_it : bss_info_conf_list) {
        ...
        if (!assign_auth_encr_parameters(conf, vap_id, bss_it.authentication_type,
                                         bss_it.encryption_type)) {
                                  ...
                                  std::string &bridge, bool vbss)
{
    ...
    assign_auth_encr_parameters(conf, ifname, bss_conf.authentication_type,
                                bss_conf.encryption_type);
    if (!bridge.empty()) {
