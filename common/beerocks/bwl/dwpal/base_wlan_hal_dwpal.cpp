...
bool base_wlan_hal_dwpal::attach_ctrl_interface(int vap_id)
{
    ...
    int result = dwpal_hostap_interface_attach(&m_dwpal_ctx[ctx_index], vap_name.c_str(), nullptr);

    if ((result == 0) && m_dwpal_ctx[ctx_index]) {
        LOG(DEBUG) << "dwpal_hostap_interface_attach() success for: " << vap_name;
    } else if (result != 0) {
        LOG(ERROR) << "dwpal_hostap_interface_attach() failed for: " << vap_name;
        return false;
    } else {
        LOG(ERROR) << "dwpal_hostap_interface_attach() returns 0 for: " << vap_name
                   << " but context is NULL!";
        return false;
    }
