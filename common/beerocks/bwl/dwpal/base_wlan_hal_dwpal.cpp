...
bool base_wlan_hal_dwpal::fsm_setup()
{
    config()
        ...
        .entry([&](const void *args) -> bool {
            ...
            for (int i = eDwpalIfaceContextIndexes::IFACE_CONTEXT_INDEX_MIN;
                 i < eDwpalIfaceContextIndexes::IFACE_CONTEXT_INDEX_MAX; i++) {
                if (m_dwpal_ctx[i]) {
...
bool base_wlan_hal_dwpal::dwpal_send_cmd(const std::string &cmd, int vap_id)
{
    ...
    if (ctx_index < eDwpalIfaceContextIndexes::IFACE_CONTEXT_INDEX_MIN ||
        ctx_index >= eDwpalIfaceContextIndexes::IFACE_CONTEXT_INDEX_MAX) {
