...
{
    LOG(ERROR) << "Anant dwpald recevied clb" << ifname << op_code;
    return 0;
}
...
bool base_wlan_hal_dwpal::attach_ctrl_interface(int vap_id)
{
    ...
    LOG(ERROR) << "Anant dwpald hostap attach" << ifname.c_str() << "return value" << dwpald_hostap_attach(ifname.c_str(),1,my_event,0);
    ...
    do {
        ...
        if (status == DWPAL_FAILURE) {
            ...
            if (!strncmp(opCode, "WPS-ENROLLEE-SEEN", sizeof(opCode)) ||
                !strncmp(opCode, "LTQ-REPORT-EVENT", sizeof(opCode)) ||
                !strncmp(opCode, "AP-STA-POSSIBLE-PSK-MISMATCH", sizeof(opCode))) {
