...
static void get_he_capa(const uint8_t *data, uint8_t len, sChannelScanResults &results)
{
    if (len <= 7) {
    ...
    /**
     * ToDo:
     * Add wifi capabilities to ChannelScanResults
     */
...
static void get_he_oper(const uint8_t *data, uint8_t len, sChannelScanResults &results)
{
    if (len <= 4) {
    ...
    if (results.operating_frequency_band ==
        ...
        if (len > 9) {
            ...
            case 1: {
                if (data[8]) {
                    int center_freq_seg0 = data[7], center_freq_seg1 = data[8];
            ...
            LOG(ERROR) << "VHT oper info present bit is on, by len is " << len;
        }
        results.supported_standards.push_back(eChannelScanResultStandards::eStandard_802_11ax);
        results.operating_standards = eChannelScanResultStandards::eStandard_802_11ax;
    }
    ...
    while (ielen >= 2 && ielen >= ie[1]) {
        ...
        case ie_type::TYPE_EXTENISON: {
            ...
            if (data[0] == ie_id_extension_values::TYPE_EXT_HE_CAPABILITIES) {
                get_he_capa(data, length, results);
            } else if (data[0] == ie_id_extension_values::TYPE_EXT_HE_OPERATION) {
                get_he_oper(data, length, results);
            }
        } break;
...
bool mon_wlan_hal_dwpal::channel_scan_dump_results(bool reset_scan_flags)
{
    if (reset_scan_flags) {
        m_nl_seq                        = 0;
        m_scan_was_triggered_internally = true;
