...
{
    LOG(ERROR) << __func__ << " - opcode: |" << opcode << "|";
    ...
    case Event::AP_Sta_Possible_Psk_Mismatch: {
        ...
        LOG(ERROR) << "sta connection failure: offending STA mac: " << msg->sta_mac;
        ...
        std::string bssid = m_radio_info.available_vaps[iface_id.vap_id].mac;

        msg->bssid = tlvf::mac_from_string(bssid);
        LOG(ERROR) << "sta connection failure: interface bssid: " << msg->bssid;
