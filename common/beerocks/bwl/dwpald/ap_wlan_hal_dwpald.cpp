...
int ap_wlan_hal_dwpal::hap_evt_acs_completed_clb(char *ifname, char *op_code, char *buffer,
                                                 size_t bufLen)
{
    LOG(DEBUG) << buffer;
    auto event                         = dwpal_to_bwl_event(op_code);
    auto csa_event_filtering_timestamp = get_csa_event_filtering_timestamp();
    bool drop_csa                      = get_drop_csa();

    if (event == ap_wlan_hal_dwpal::Event::CSA_Finished) {
        if (std::chrono::steady_clock::now() <
                (csa_event_filtering_timestamp +
        ...
        csa_event_filtering_timestamp = std::chrono::steady_clock::now();
    ...
    std::string channelStr =
        (event == ap_wlan_hal_dwpal::Event::CSA_Finished) ? "Channel=" : "channel=";
    size_t numOfValidArgs[8]      = {0};
    FieldsToParse fieldsToParse[] = {
        {NULL /*opCode*/, &numOfValidArgs[0], DWPAL_STR_PARAM, NULL, 0},
    ...
    if (dwpal_string_to_struct_parse(buffer, bufLen, fieldsToParse, sizeof(radio_info)) ==
        DWPAL_FAILURE) {
        LOG(ERROR) << "DWPAL parse error ==> Abort";
        return -1;
    }
    ...
    LOG(DEBUG) << "numOfValidArgs[3]= " << numOfValidArgs[3]
               << " OperatingChannelBandwidt= " << (int)radio_info.bandwidth;
    LOG(DEBUG) << "numOfValidArgs[4]= " << numOfValidArgs[4]
               << " ExtensionChannel= " << (int)radio_info.channel_ext_above;
    LOG(DEBUG) << "numOfValidArgs[5]= " << numOfValidArgs[5]
               << " cf1= " << (int)radio_info.vht_center_freq;
    LOG(DEBUG) << "numOfValidArgs[6]= " << numOfValidArgs[6]
               << " dfs_chan= " << (int)radio_info.is_dfs_channel;
    ...
    radio_info.last_csa_sw_reason = chanSwReason;

    // Update the radio information structure
    if (son::wireless_utils::which_freq(radio_info.channel) == beerocks::eFreqType::FREQ_5G) {
...
int ap_wlan_hal_dwpal::hap_evt_ap_csa_finished_clb(char *ifname, char *op_code, char *msg,
                                                   size_t len)
{
    return hap_evt_acs_completed_clb(ifname, op_code, msg, len);
}
...
int ap_wlan_hal_dwpal::hap_evt_acs_started_clb(char *ifname, char *op_code, char *msg, size_t len)
{
    LOG(ERROR) << "Either entity shall register not for " << op_code
               << "event or if register then override base class callback";
    return 0;
    ...
    bool &is_finished_all_clients, std::chrono::steady_clock::time_point max_iteration_timeout)
{
    ...
    while (m_vap_id_in_progress != INVALID_VAP_ID) {
        ...
        do {
            ...
            event_queue_push(ap_wlan_hal_dwpal::Event::STA_Connected,
...
int ap_wlan_hal_dwpal::filter_bss_msg(char *buffer, int bufLen, const std::string &opcode)
{
    ...
    if (!m_hal_conf.monitored_BSSs.empty()) {
        if (event == ap_wlan_hal_dwpal::Event::STA_Connected ||
...
bool ap_wlan_hal_dwpal::process_dwpal_event(char *buffer, int bufLen, const std::string &opcode)
{
    ...
    case ap_wlan_hal_dwpal::Event::STA_Disconnected: {
        ...
        event_queue_push(ap_wlan_hal_dwpal::Event::STA_Disconnected,
                         msg_buff); // send message to the AP manager
        break;
    }
    ...
    case ap_wlan_hal_dwpal::Event::AP_Disabled: {
        ...
        event_queue_push(ap_wlan_hal_dwpal::Event::AP_Disabled,
                         msg_buff); // send message to the AP manager
                                                        ...
                                                        size_t bufLen)
{
    ...
    auto msg_buff =
        ALLOC_SMART_BUFFER(sizeof(sACTION_APMANAGER_CLIENT_RX_RSSI_MEASUREMENT_RESPONSE));
    auto msg =
        reinterpret_cast<sACTION_APMANAGER_CLIENT_RX_RSSI_MEASUREMENT_RESPONSE *>(msg_buff.get());
    LOG_IF(!msg, FATAL) << "Memory allocation failed!";
    ...
    auto rssiVec                     = beerocks::string_utils::str_split(rssi, ' ');
    auto snrVec                      = beerocks::string_utils::str_split(rssi, ' ');
    auto unassoc_measure_start       = get_unassoc_measure_start();
    auto unassoc_measure_window_size = get_unassoc_measure_window_size();
    auto unassoc_measure_delay       = get_unassoc_measure_delay();
...
int ap_wlan_hal_dwpal::hap_evt_bss_tm_query_clb(char *ifname, char *op_code, char *buffer,
                                                size_t bufLen)
{
    parsed_line_t parsed_obj;
    parse_event(buffer, parsed_obj);
    ...
    // current VAP, there is no need to notify the upper layer.
    // disassoc_timer_btt = 0 valid_int_btt=2 (200ms) reason=0 (not specified)
    sta_bss_steer(client_mac_str, bssid, op_class, radio_info.channel, 0, 2, 0);
    return 0;
}

int ap_wlan_hal_dwpal::hap_evt_ap_disabled_clb(char *ifname, char *op_code, char *buffer,
                                               size_t bufLen)
{
    auto msg_buff = ALLOC_SMART_BUFFER(sizeof(sHOSTAP_DISABLED_NOTIFICATION));
    auto msg      = reinterpret_cast<sHOSTAP_DISABLED_NOTIFICATION *>(msg_buff.get());
                                                     ...
                                                     size_t bufLen)
{
    ...
    auto msg_buff =
        ALLOC_SMART_BUFFER(sizeof(sACTION_APMANAGER_HOSTAP_DFS_CAC_COMPLETED_NOTIFICATION));
    auto msg =
        reinterpret_cast<sACTION_APMANAGER_HOSTAP_DFS_CAC_COMPLETED_NOTIFICATION *>(msg_buff.get());
    LOG_IF(!msg, FATAL) << "Memory allocation failed!";
    // Initialize the message
    memset(msg_buff.get(), 0, sizeof(sACTION_APMANAGER_HOSTAP_DFS_CAC_COMPLETED_NOTIFICATION));
...
int ap_wlan_hal_dwpal::hap_evt_dfs_cac_start_clb(char *ifname, char *op_code, char *buffer,
                                                 size_t bufLen)
{
    LOG(DEBUG) << buffer;
    ...
    auto msg_buff =
        ALLOC_SMART_BUFFER(sizeof(sACTION_APMANAGER_HOSTAP_DFS_CAC_STARTED_NOTIFICATION));
    auto msg =
        reinterpret_cast<sACTION_APMANAGER_HOSTAP_DFS_CAC_STARTED_NOTIFICATION *>(msg_buff.get());
    LOG_IF(!msg, FATAL) << "Memory allocation failed!";
    // Initialize the message
    memset(msg_buff.get(), 0, sizeof(sACTION_APMANAGER_HOSTAP_DFS_CAC_STARTED_NOTIFICATION));
...
int ap_wlan_hal_dwpal::hap_evt_bss_tm_resp_clb(char *ifname, char *op_code, char *buffer,
                                               size_t bufLen)
{
    // TODO: Change to HAL objects
    auto msg_buff = ALLOC_SMART_BUFFER(sizeof(sACTION_APMANAGER_CLIENT_BSS_STEER_RESPONSE));
    ...
    if (dwpal_string_to_struct_parse(buffer, bufLen, fieldsToParse,
                                     sizeof(vap_name) + sizeof(MACAddress) + sizeof(status_code)) ==
        DWPAL_FAILURE) {
        LOG(ERROR) << "DWPAL parse error ==> Abort";
        return false;
    }
                                                    ...
                                                    size_t bufLen)
{
    ...
    if (dwpal_string_to_struct_parse(buffer, bufLen, fieldsToParse, sizeof(int)) == DWPAL_FAILURE) {
        LOG(ERROR) << "DWPAL parse error ==> Abort";
        return false;
    }
...
int ap_wlan_hal_dwpal::hap_evt_ap_enabled_clb(char *ifname, char *op_code, char *buffer,
                                              size_t bufLen)
{
    auto msg_buff = ALLOC_SMART_BUFFER(sizeof(sHOSTAP_ENABLED_NOTIFICATION));
    auto msg      = reinterpret_cast<sHOSTAP_ENABLED_NOTIFICATION *>(msg_buff.get());
...
int ap_wlan_hal_dwpal::hap_evt_ap_action_frame_received_clb(char *ifname, char *op_code,
                                                    ...
                                                    size_t bufLen)
{
    ...
    FieldsToParse fieldsToParse[] = {
        ...
        {(void *)RRM_CAP, &numOfValidArgs[4], DWPAL_INT_HEX_ARRAY_PARAM,
         "rrm_cap=", sizeof(RRM_CAP)},
        {(void *)HT_MCS, &numOfValidArgs[5], DWPAL_INT_HEX_ARRAY_PARAM, "HT_MCS=", sizeof(HT_MCS)},
        {(void *)VHT_MCS, &numOfValidArgs[6], DWPAL_INT_HEX_ARRAY_PARAM,
         "VHT_MCS=", sizeof(VHT_MCS)},
        {(void *)ht_cap, &numOfValidArgs[7], DWPAL_STR_PARAM, "HT_CAP=", sizeof(ht_cap)},
        {(void *)vht_cap, &numOfValidArgs[8], DWPAL_STR_PARAM, "VHT_CAP=", sizeof(vht_cap)},
        {(void *)assoc_req, &numOfValidArgs[9], DWPAL_STR_PARAM, "assoc_req=", sizeof(assoc_req)},
        {(void *)&msg->params.capabilities.btm_supported, &numOfValidArgs[10], DWPAL_CHAR_PARAM,
         "btm_supported=", 0},
        {(void *)&msg->params.capabilities.cell_capa, &numOfValidArgs[11], DWPAL_CHAR_PARAM,
         "cell_capa=", 0},
        {(void *)&msg->params.capabilities.band_2g_capable, &numOfValidArgs[12], DWPAL_CHAR_PARAM,
         "band_2g_capable=", 0},
        {(void *)&msg->params.capabilities.band_5g_capable, &numOfValidArgs[13], DWPAL_CHAR_PARAM,
         "band_5g_capable=", 0},
        {(void *)&msg->params.capabilities.rrm_supported, &numOfValidArgs[14], DWPAL_CHAR_PARAM,
         "rrm_supported=", 0},
        {(void *)&max_ch_width, &numOfValidArgs[15], DWPAL_CHAR_PARAM, "max_ch_width=", 0},
        ...
        {(void *)&msg->params.capabilities.max_tx_power, &numOfValidArgs[19], DWPAL_CHAR_PARAM,
         "max_tx_power=", 0},
        {(void *)&msg->params.capabilities.mumimo_supported, &numOfValidArgs[20], DWPAL_CHAR_PARAM,
         "mu_mimo=", 0},
        {(void *)&msg->params.multi_ap_profile, &numOfValidArgs[21], DWPAL_INT_PARAM,
         "multi_ap_profile=", 0},
        /* Must be at the end */
    ...
    auto completed_vaps  = get_completed_vap();
    auto handled_clients = get_handled_clients();
    // No need to store clients forever - may cause very big memory usage
...
#define EVENT(event) (char *)event, sizeof(event) - 1, hapd_evt_clb
#if 0
    {EVENT("AP-ENABLED"), hap_evt_ap_enabled_clb},
    {EVENT("AP-DISABLED"), hap_evt_ap_disabled_clb},
    {EVENT("AP-STA-CONNECTED"), hap_evt_ap_sta_connected_clb},
    {EVENT("AP-STA-DISCONNECTED"), hap_evt_ap_sta_disconnected_clb},
    {EVENT("UNCONNECTED-STA-RSSI"), hap_evt_unconnected_sta_rssi_clb},
    {EVENT("INTERFACE-ENABLED"), hap_evt_interface_enabled_clb},
    {EVENT("INTERFACE-DISABLED"), hap_evt_interface_disabled_clb},
    {EVENT("ACS-STARTED"), hap_evt_acs_started_clb},
    {EVENT("ACS-COMPLETED"), hap_evt_acs_completed_clb},
    {EVENT("ACS-FAILED"), hap_evt_acs_failed_clb},
    {EVENT("AP-CSA-FINISHED"), hap_evt_ap_csa_finished_clb},
    {EVENT("BSS-TM-QUERY"), hap_evt_bss_tm_query_clb},
    {EVENT("BSS-TM-RESP"), hap_evt_bss_tm_resp_clb},
    {EVENT("DFS-CAC-START"), hap_evt_dfs_cac_start_clb},
    {EVENT("DFS-CAC-COMPLETED"), hap_evt_dfs_cac_completed_clb},
    {EVENT("DFS-NOP-FINISHED"), hap_evt_dfs_nop_finished_clb},
    {EVENT("LTQ-SOFTBLOCK-DROP"), hap_evt_ltq_softblock_drop_clb},
    {EVENT("AP-ACTION-FRAME-RECEIVED"), hap_evt_ap_action_frame_received_clb},
    {EVENT("AP-STA-POSSIBLE-PSK-MISMATCH"), hap_evt_ap_sta_possible_psk_mismatch_clb}};
static int hap_evt_callback(char *ifname, char *op_code, char *buffer, size_t len)
{
    auto result = filter_bss_msg(buffer, len, op_code);
    if (result <= 0) {
        return result;
    }
    auto event = dwpal_to_bwl_event(op_code);
    switch (event) {
    case ap_wlan_hal_dwpal::Event::ACS_Failed: {
        hap_evt_acs_failed_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::Interface_Disabled: {
        hap_evt_interface_disabled_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::ACS_Completed: {
        hap_evt_acs_completed_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::CSA_Finished: {
        hap_evt_ap_csa_finished_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::STA_Disconnected: {
        hap_evt_ap_sta_disconnected_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::STA_Connected: {
        hap_evt_ap_sta_connected_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::STA_Unassoc_RSSI: {
        hap_evt_unconnected_sta_rssi_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::STA_Softblock_Drop: {
        hap_evt_ltq_softblock_drop_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::BSS_TM_Query: {
        hap_evt_bss_tm_query_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::BSS_TM_Response: {
        hap_evt_bss_tm_resp_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::DFS_CAC_Started: {
        hap_evt_dfs_cac_start_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::DFS_CAC_Completed: {
        hap_evt_dfs_cac_completed_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::DFS_NOP_Finished: {
        hap_evt_dfs_nop_finished_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::AP_Disabled: {
        hap_evt_ap_disabled_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::AP_Enabled: {
        hap_evt_ap_enabled_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::MGMT_Frame: {
        hap_evt_ap_action_frame_received_clb(ifname, op_code, buffer, len);
    } break;
    case ap_wlan_hal_dwpal::Event::AP_Sta_Possible_Psk_Mismatch: {
        hap_evt_ap_sta_possible_psk_mismatch_clb(ifname, op_code, buffer, len);
    } break;
    default: {
...
void ap_wlan_hal_dwpal::hostap_attach(char *ifname)
{
    ...
    static __thread std::function<int(char *ifname, char *op_code, char *buffer, size_t len)> hapd_evt_callback;
    hapd_evt_callback = [&](char *ifname, char *op_code, char *buffer, size_t len) -> int {
        auto result = filter_bss_msg(buffer, len, op_code);
    if (result <= 0) {
        return result;
    }
    ...
    auto hapd_evt_clb = [](char *ifname, char *op_code, char *msg, size_t len) -> int { return hapd_evt_callback(ifname,op_code,msg,len); };
    #endif
