...
/* 
 * Currently, DSCP-WMM configuration is applied to WiFI driver by qos_map_set hostapd infra.
 * There are some limitations in this infra. There can be upto 21 optional DSCP exceptions
 * other than 8 DSCP Low and High value pairs. So, we can only handle 21 exceptions case in
 * DSCP-WMM mapping.  
*/

    //Copy DSCP - WMM mapping details
    std::copy(data, data + beerocks::message::DSCP_MAPPING_LIST_LENGTH, dscp);

    //Initialise exception and range details with invalid value as 255
    ...
    // If a dscp-wmm value is not present in the range, then it must be considered as exception
    for (i = 0, j = 0; i < 64; i++) {
        for (k = 0; k < 8; k++) {
    ...
    // Constructing Hostapd command with range and exceptions
    ...
    for (const auto &vap : m_radio_info.available_vaps) {
        ...
        std::string vap_name = beerocks::utils::get_iface_string_from_iface_vap_ids(
            m_radio_info.iface_name, vap.first);
        cmd = "SET_QOS_MAP_SET " + vap_name + " " + ss.str();
        LOG(DEBUG) << "CMD : " << cmd;
        if (!dwpal_send_cmd(cmd)) {
            LOG(ERROR) << "set_qos_map_set failed! " << vap_name;
        }
    }
