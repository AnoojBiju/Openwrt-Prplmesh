      ...
      beerocks::beerocks_fsm<dummy_fsm_state, dummy_fsm_event>(dummy_fsm_state::Delay)
{
    ...
    if (unlink(m_dummy_event_file.c_str()) == -1) {
        if (errno != ENOENT) { // Do NOT fail if the file doesn't exists
            LOG(FATAL) << "Failed removing previous events FIFO: " << strerror(errno);
        }
    }
...
bool base_wlan_hal_dummy::process_ext_events()
{
    ...
    if (read_bytes <= 0) {
        LOG(FATAL) << "Failed reading event: " << strerror(errno);
    }
    ...
    // Ignore the last byte from the read stream
