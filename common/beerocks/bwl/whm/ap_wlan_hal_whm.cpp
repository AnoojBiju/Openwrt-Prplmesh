...
bool ap_wlan_hal_whm::process_wpaCtrl_events(const std::string &interface,
                                             const beerocks::wbapi::AmbiorixVariant *event_data)
{
    std::string event_str;
    if (!event_data->read_child<>(event_str, "eventData") || event_str.empty()) {
        LOG(WARNING) << "Unable to retrieve wpaCtrl event from pwhm notification";
        return false;
    }
    LOG(WARNING) << "wpaCtrl event " << event_str;

    parsed_obj_map_t event_obj;
    map_event_obj_parser(event_str, event_obj);

    std::string opcode;
    if (!(event_obj.find(bwl::EVENT_KEYLESS_PARAM_OPCODE) != event_obj.end() &&
          !(opcode = event_obj[bwl::EVENT_KEYLESS_PARAM_OPCODE]).empty())) {
        return false;
    }
    ...
    case Event::AP_Sta_Possible_Psk_Mismatch: {
        ...
        msg->sta_mac = tlvf::mac_from_string(event_obj[bwl::EVENT_KEYLESS_PARAM_MAC]);
