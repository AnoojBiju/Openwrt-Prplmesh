      ...
      m_iso_nl80211_client(nl80211_client_factory::create_instance())
{
    ...
    m_iso_nl80211_whm_client = static_cast<bwl::nl80211_client_whm *>(m_iso_nl80211_client.get());

    m_fds_ext_events.push_back(m_iso_nl80211_whm_client->get_fd());
}
...
bool base_wlan_hal_whm::process_ext_events(int fd)
{
    if (m_ambiorix_cl.get_fd() == fd) {
        ...
        m_ambiorix_cl.read_signal();
    } else if (m_iso_nl80211_whm_client && m_iso_nl80211_whm_client->get_fd() == fd) {
        m_iso_nl80211_whm_client->read();
    }
    return true;
}
