...
void ApManager::handle_virtual_bss_request(ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    auto send_virtual_bss_response = [this](const sMacAddr &radio_uid, bool success, const sMacAddr &bssid) {
        auto cmdu_tx_header = cmdu_tx.create(0, ieee1905_1::eMessageType::VIRTUAL_BSS_RESPONSE_MESSAGE);
        if (!cmdu_tx_header) {
    ...
    if (virtual_bss_creation_tlv) {
        ...
        if (virtual_bss_creation_tlv->client_assoc()) {
            ...
            bwl::sKeyInfo pairwise_key = {
                ...
                .key         = {virtual_bss_creation_tlv->ptk(), virtual_bss_creation_tlv->ptk() + virtual_bss_creation_tlv->key_length()},
                .key_seq     = {},
            ...
            bwl::sKeyInfo group_key = {
                ...
                .key         = {virtual_bss_creation_tlv->gtk(), virtual_bss_creation_tlv->gtk() + virtual_bss_creation_tlv->key_length()},
                .key_seq     = {},
            ...
            group_key_seq.insert(group_key_seq.end(), group_tx_pn_ptr, group_tx_pn_ptr + group_key_seq_len);
...
{
    LOG(DEBUG) << "Received move preparation request";
    auto cmdu_tx_header = cmdu_tx.create(0, ieee1905_1::eMessageType::VIRTUAL_BSS_MOVE_PREPARATION_RESPONSE_MESSAGE);
    if (!cmdu_tx_header) {
...
void ApManager::handle_vbss_security_request(ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    memcpy(&client_security_context_tlv->tx_packet_num(), pairwise_key.key_seq.data(), pairwise_key.key_seq.size() );
    ...
    memcpy(&client_security_context_tlv->group_tx_packet_num(), group_key.key_seq.data(), group_key.key_seq.size() );
                                ...
                                std::string &bridge, bool vbss)
{
    ...
    auto &ext_event_fd = m_ap_hal_ext_events.back();

    if (ext_event_fd < 0) {
        ext_event_fd = beerocks::net::FileDescriptor::invalid_descriptor;
        LOG(ERROR) << "Invalid file descriptor for external events!";
        return false;
    }
    if (!m_event_loop->register_handlers(ext_event_fd, ext_events_handlers)) {
        LOG(ERROR)
 ...
 */
std::string ApManager::get_vbss_interface_name(sMacAddr bssid){
