...
bool slave_thread::agent_fsm()
{
    ...
    case STATE_OPERATIONAL: {
        ...
        LOG(DEBUG) << "Badhri db->device_conf.certification_mode = "
                   << db->device_conf.certification_mode;
        LOG(DEBUG) << "Badhri db->device_conf.on_boot_scan = " << db->device_conf.on_boot_scan;
        if (db->device_conf.certification_mode && db->device_conf.on_boot_scan > 0) {
            const auto &radio_to_scan         = db->device_conf.on_boot_scan - 1;
