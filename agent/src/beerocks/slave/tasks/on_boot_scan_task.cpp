...
void OnBootScanTask::handle_event(uint8_t event_enum_value, const void *event_obj)
{
...
bool OnBootScanTask::handle_cmdu(ieee1905_1::CmduMessageRx &cmdu_rx, uint32_t iface_index,
                                 ...
                                 std::shared_ptr<beerocks_header> beerocks_header)
{
    ...
    case ieee1905_1::eMessageType::CHANNEL_SCAN_REQUEST_MESSAGE: {
        ...
        LOG(DEBUG) << "Badhri src_mac: " << src_mac << " dst_mac: " << dst_mac;
...
bool OnBootScanTask::handle_vendor_specific(ieee1905_1::CmduMessageRx &cmdu_rx,
...
bool OnBootScanTask::is_scan_request_finished(const std::shared_ptr<sScanRequest> request)
{
...
bool OnBootScanTask::abort_scan_request(const std::shared_ptr<sScanRequest> request)
{
...
bool OnBootScanTask::trigger_next_radio_scan(const std::shared_ptr<sScanRequest> request)
{
    /**
...
bool OnBootScanTask::set_radio_scan_status(const std::shared_ptr<sRadioScan> radio_scan_info,
...
bool OnBootScanTask::trigger_radio_scan(const std::string &radio_iface,
...
bool OnBootScanTask::store_radio_scan_result(const std::shared_ptr<sScanRequest> request,
...
bool OnBootScanTask::handle_on_boot_scan_request(ieee1905_1::CmduMessageRx &cmdu_rx,
                                                 const sMacAddr &src_mac, const sMacAddr &radio_mac,
                                                 const bool &send_results)
{
    ...
    new_request = std::shared_ptr<sScanRequest>(new sScanRequest(), [](sScanRequest *ptr) {
    ...
    sAgentRequestedInfo request_info;
    request_info.src_mac = src_mac;

    new_request->request_info         = std::make_shared<sAgentRequestedInfo>(request_info);
    ...
    if (radio->wifi_channel.get_freq_type() == beerocks::FREQ_5G) {
        operating_classes = {115, 118, 121, 122, 124};
    ...
    new_radio_scan = std::shared_ptr<sRadioScan>(new sRadioScan(), [](sRadioScan *ptr) {
...
bool OnBootScanTask::send_channel_scan_report_to_controller(
    const std::shared_ptr<sScanRequest> request)
{
    ...
    const auto request_info = std::static_pointer_cast<sAgentRequestedInfo>(request->request_info);
    if (!request_info) {
        LOG(ERROR) << "Unable to cast request info as sAgentRequestedInfo";
        return false;
    }
...
OnBootScanTask::get_scan_results_for_request(const std::shared_ptr<sScanRequest> request)
{
    ...
    for (const auto &radio_scan_iter : request->radio_scans) {
        ...
        const auto &stored_scan_results_map = radio->on_boot_scan_results;
