...
void CacFsm::db_store_cac_status(std::shared_ptr<sCacStartedNotification> cac_started)
{
    ...
    channel_record->second.set_dfs_state(beerocks_message::eDfsState::ACTIVE_CAC);
}
...
// cac task ///
///////////////
CacTask::CacTask(TaskPoolInterface &task_pool, BackhaulManager &backhaul_manager,
                 ieee1905_1::CmduMessageTx &cmdu_tx)
...
std::vector<eTaskEvent> CacTask::get_task_event_list() const
{
...
void CacTask::handle_event(uint8_t event_enum_value, const void *event_obj) {}

void CacTask::handle_event(eTaskEvent event, std::shared_ptr<void> event_obj)
{
    LOG(DEBUG) << "got event to handle: " << static_cast<uint8_t>(event);
...
bool CacTask::handle_cmdu(ieee1905_1::CmduMessageRx &cmdu_rx, uint32_t iface_index,
