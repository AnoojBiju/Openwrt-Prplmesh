...
bool on_boot_scan_enabled              = false;
...
void ChannelScanTask::work()
{
    ...
    if (m_current_scan_info.is_scan_currently_running) {
        ...
        if (!on_boot_scan_enabled && current_scan_request->ready_to_send_report) {
...
ChannelScanTask::get_scan_results_for_request(const std::shared_ptr<sScanRequest> request)
{
    ...
    for (const auto &radio_scan_iter : request->radio_scans) {
        ...
        const auto &stored_scan_results_map = on_boot_scan_enabled
                                                  ? radio->on_boot_scan_results
                                                  : radio->on_demand_channel_scan_results;
