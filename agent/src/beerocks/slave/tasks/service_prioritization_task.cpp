    ...
    std::list<bpl::ServicePrioritizationUtils::sInterfaceTagInfo> *iface_tag_info_list)
{
    ...
    LOG(DEBUG) << "collecting Interface details";
    UTILS_SLEEP_MSEC(500);

    LOG(DEBUG) << "bridge name = " << db->bridge.iface_name;
    UTILS_SLEEP_MSEC(500);
    ...
    LOG(DEBUG) << "collecting Interface details : 1";
    UTILS_SLEEP_MSEC(500);
    ...
    LOG(DEBUG) << "collecting Interface details : 2";
    UTILS_SLEEP_MSEC(500);
    ...
    if (!db->device_conf.local_gw && !db->backhaul.selected_iface_name.empty() &&
        ...
        if (db->backhaul.bssid_multi_ap_profile > 1) {
            iface.tag_info = bpl::ServicePrioritizationUtils::ePortMode::TAGGED_PORT_PRIMARY_TAGGED;
        } else {
    ...
    LOG(DEBUG) << "collecting Interface details : 3";
    UTILS_SLEEP_MSEC(500);
    ...
    LOG(DEBUG) << "collecting Interface details : 4";
    UTILS_SLEEP_MSEC(500);
    LOG(DEBUG) << "Interface details are collected";
    for (auto itr = iface_tag_info_list->rbegin(); itr != iface_tag_info_list->rend(); itr++) {
...
bool ServicePrioritizationTask::qos_setup_up_map()
{
    ...
    qos_flush_setup();
    pcp = AgentDB::get()->traffic_separation.default_pcp;
    ServicePrioritizationTask::gather_iface_details(&iface_list);
