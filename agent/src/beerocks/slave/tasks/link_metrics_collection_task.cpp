                                                           ...
                                                           const sMacAddr &src_mac)
{
    ...
    for (const auto &response : m_ap_metric_response) {
        ...
        for (auto &qos_control_params : response.sta_wifi_6_status) {
            ...
            uint8_t tid_queue_size_list_len                 = 8;
            for (uint8_t n = 0; n < tid_queue_size_list_len; n++) {
                auto sta_wifi6_status_report =
                    sta_wifi6_status_report_response_tlv->create_tid_queue_size_list();

                if (!sta_wifi6_status_report) {
                    LOG(ERROR) << "create_tid_queue_size_list() has failed!";
                }

                sta_wifi6_status_report->tid()        = n;
                sta_wifi6_status_report->queue_size() = qos_control_params.tid_queue_size[n];

                if (!sta_wifi6_status_report_response_tlv->add_tid_queue_size_list(
                        sta_wifi6_status_report)) {
