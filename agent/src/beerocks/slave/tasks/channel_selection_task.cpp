    ...
    for (const auto &channel_iter : radio->channels_list) {
        ...
        for (auto &bw_info : channel_info.supported_bw_list) {
            ...
            if (cumulative_preference == 0) {
                ...
                LOG(WARNING) << "Either radio or controller preference is Non-Operable, skipping";
                continue;
            }
            ...
            if (primary_preference < best_channel.preference_score) {
                ...
                LOG(WARNING) << "Found preference is lower then the best, skipping.";
                LOG(INFO) << primary_preference << "<" << best_channel.preference_score;
                continue;
                ...
                if (bandwidth <= best_channel.bw) {
                    ...
                    LOG(WARNING) << "Found bandwidth is lower or equal to the best, skipping.";
                    LOG(INFO) << utils::convert_bandwidth_to_int(best_channel.bw)
                              << "<=" << utils::convert_bandwidth_to_int(bandwidth);
                    continue;
                }
            }
