...
class ApMetricsResponseR3(PrplMeshBaseTest):
    '''
    ...
    def runTest(self):
        ...
        response = self.check_cmdu_type_single("AP metrics response",
                                               ...
                                               ['AP_METRICS_RESPONSE_MESSAGE'], agent2.mac,
                                               controller.mac, mid)
        debug("Check AP Metrics Response message has STA WIFI 6 Status report TLV")
        ap_metrics_3 = self.check_cmdu_has_tlv_single(response,
                                                      self.ieee1905['eTlvTypeMap']['TLV_AP_METRIC'])
        if ap_metrics_3:
            if ap_metrics_3.ap_metrics_bssid != vap2.bssid:
                self.fail("AP metrics response with wrong BSSID {} instead of {}".format(
                    ap_metrics_3.ap_metrics_bssid, vap2.bssid))

        debug("Check AP metrics response has STA Wifi 6 status")
        sta_report = self.check_cmdu_has_tlv_single(response,
                                                    self.ieee1905['eTlvTypeMap']
