...
class tlvTeamsMembers : public BaseClass
{
    public:
        ...
        uint8_t& team_id();
        std::tuple<bool, cTeamDetails&> team_details(size_t idx);
        std::shared_ptr<cTeamDetails> create_team_details();
        bool add_team_details(std::shared_ptr<cTeamDetails> ptr);
    ...
    private:
        ...
        cTeamDetails* m_team_details = nullptr;
        size_t m_team_details_idx__ = 0;
        std::vector<std::shared_ptr<cTeamDetails>> m_team_details_vector;
...
class cTeamDetails : public BaseClass
{
    public:
        ...
        uint8_t& num_dev();
        std::tuple<bool, cDevDetails&> dev_details(size_t idx);
        std::shared_ptr<cDevDetails> create_dev_details();
        bool add_dev_details(std::shared_ptr<cDevDetails> ptr);
        void class_swap() override;
        bool finalize() override;
        static size_t get_initial_size();
    ...
    private:
        ...
        int m_lock_order_counter__ = 0;
        uint8_t* m_num_dev = nullptr;
        cDevDetails* m_dev_details = nullptr;
        size_t m_dev_details_idx__ = 0;
        std::vector<std::shared_ptr<cDevDetails>> m_dev_details_vector;
        bool m_lock_allocation__ = false;
};

class cDevDetails : public BaseClass
{
    public:
        cDevDetails(uint8_t* buff, size_t buff_len, bool parse = false);
        explicit cDevDetails(std::shared_ptr<BaseClass> base, bool parse = false);
        ~cDevDetails();
        ...
        } __attribute__((packed)) sDevDetails;
        ...
        //Consist of developer work location and number of years experience.
        sDevDetails& work_exp();
        uint8_t& age();
        //Previous company list length.
    ...
    private:
        ...
        int m_lock_order_counter__ = 0;
        sDevDetails* m_work_exp = nullptr;
