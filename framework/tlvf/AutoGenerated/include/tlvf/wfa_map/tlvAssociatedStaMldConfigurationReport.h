...
class tlvAssociatedStaMldConfigurationReport : public BaseClass
{
    public:
        ...
        std::tuple<bool, cAffliatedsta&> affliated_sta(size_t idx);
        std::shared_ptr<cAffliatedsta> create_affliated_sta();
        bool add_affliated_sta(std::shared_ptr<cAffliatedsta> ptr);
    ...
    private:
        ...
        cAffliatedsta* m_affliated_sta = nullptr;
        size_t m_affliated_sta_idx__ = 0;
        std::vector<std::shared_ptr<cAffliatedsta>> m_affliated_sta_vector;
        bool m_lock_allocation__ = false;
};

class cAffliatedsta : public BaseClass
{
    public:
        cAffliatedsta(uint8_t* buff, size_t buff_len, bool parse = false);
        explicit cAffliatedsta(std::shared_ptr<BaseClass> base, bool parse = false);
        ~cAffliatedsta();
    ...
    private:
        ...
        sMacAddr* m_affliated_sta_mac_addr = nullptr;
        uint8_t* m_reserved = nullptr;
        size_t m_reserved_idx__ = 0;
        int m_lock_order_counter__ = 0;
};
