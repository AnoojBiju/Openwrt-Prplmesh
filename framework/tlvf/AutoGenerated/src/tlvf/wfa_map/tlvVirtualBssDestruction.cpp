...
VirtualBssDestruction::VirtualBssDestruction(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
VirtualBssDestruction::VirtualBssDestruction(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
VirtualBssDestruction::~VirtualBssDestruction() {
}
const eTlvTypeMap& VirtualBssDestruction::type() {
    return (const eTlvTypeMap&)(*m_type);
}

const uint16_t& VirtualBssDestruction::length() {
    return (const uint16_t&)(*m_length);
}

const eVirtualBssSubtype& VirtualBssDestruction::subtype() {
    return (const eVirtualBssSubtype&)(*m_subtype);
}

sMacAddr& VirtualBssDestruction::radio_uid() {
    return (sMacAddr&)(*m_radio_uid);
}

sMacAddr& VirtualBssDestruction::bssid() {
    return (sMacAddr&)(*m_bssid);
}

uint8_t& VirtualBssDestruction::disassociate_client() {
    return (uint8_t&)(*m_disassociate_client);
}

void VirtualBssDestruction::class_swap()
{
...
bool VirtualBssDestruction::finalize()
{
    if (m_parse__) {
...
size_t VirtualBssDestruction::get_initial_size()
{
    size_t class_size = 0;
    ...
    class_size += sizeof(uint8_t); // disassociate_client
    return class_size;
}

bool VirtualBssDestruction::init()
{
    ...
    if (!m_parse__) *m_subtype = eVirtualBssSubtype::VIRTUAL_BSS_DESTRUCTION;
