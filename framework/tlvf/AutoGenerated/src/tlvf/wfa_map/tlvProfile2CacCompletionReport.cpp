...
uint8_t& cCacRadio::cac_completion_status() {
    return (uint8_t&)(*m_cac_completion_status);
}
...
size_t cCacRadio::get_initial_size()
{
    ...
    class_size += sizeof(uint8_t); // cac_completion_status
...
bool cCacRadio::init()
{
    ...
    m_cac_completion_status = reinterpret_cast<uint8_t*>(m_buff_ptr__);
    if (!buffPtrIncrementSafe(sizeof(uint8_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) << ") Failed!";
        return false;
    }
