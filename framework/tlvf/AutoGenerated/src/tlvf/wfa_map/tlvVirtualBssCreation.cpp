...
const eTlvTypeMap &VirtualBssCreation::type() { return (const eTlvTypeMap &)(*m_type); }

const uint16_t &VirtualBssCreation::length() { return (const uint16_t &)(*m_length); }
...
sMacAddr &VirtualBssCreation::radio_uid() { return (sMacAddr &)(*m_radio_uid); }

sMacAddr &VirtualBssCreation::bssid() { return (sMacAddr &)(*m_bssid); }

uint16_t &VirtualBssCreation::ssid_length() { return (uint16_t &)(*m_ssid_length); }
...
bool VirtualBssCreation::set_ssid(const std::string &str)
{
    return set_ssid(str.c_str(), str.size());
}
...
uint16_t &VirtualBssCreation::pass_length() { return (uint16_t &)(*m_pass_length); }
...
bool VirtualBssCreation::set_pass(const std::string &str)
{
    return set_pass(str.c_str(), str.size());
}
...
bool VirtualBssCreation::set_dpp_connector(const std::string &str)
{
    return set_dpp_connector(str.c_str(), str.size());
}
...
sMacAddr &VirtualBssCreation::client_mac() { return (sMacAddr &)(*m_client_mac); }

uint8_t &VirtualBssCreation::client_assoc() { return (uint8_t &)(*m_client_assoc); }

uint16_t &VirtualBssCreation::key_length() { return (uint16_t &)(*m_key_length); }
...
uint64_t &VirtualBssCreation::tx_packet_num() { return (uint64_t &)(*m_tx_packet_num); }

uint16_t &VirtualBssCreation::group_key_length() { return (uint16_t &)(*m_group_key_length); }
...
uint64_t &VirtualBssCreation::group_tx_packet_num() { return (uint64_t &)(*m_group_tx_packet_num); }

uint16_t &VirtualBssCreation::authentication_type() { return (uint16_t &)(*m_authentication_type); }
...
bool VirtualBssCreation::init()
{
    ...
    if (!m_parse__)
        *m_type = eTlvTypeMap::TLV_VIRTUAL_BSS;
    ...
    if (!m_parse__)
        *m_subtype = eVirtualBssSubtype::VIRTUAL_BSS_CREATION;
    if (!buffPtrIncrementSafe(sizeof(eVirtualBssSubtype))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(eVirtualBssSubtype)
                   << ") Failed!";
        return false;
    }
    if (m_length && !m_parse__) {
        (*m_length) += sizeof(eVirtualBssSubtype);
    }
    ...
    if (m_parse__) {
        tlvf_swap(16, reinterpret_cast<uint8_t *>(&ssid_length));
    }
    m_ssid_idx__ = ssid_length;
    if (!buffPtrIncrementSafe(sizeof(char) * (ssid_length))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(char) * (ssid_length)
                   << ") Failed!";
        return false;
    }
    ...
    if (m_parse__) {
        tlvf_swap(16, reinterpret_cast<uint8_t *>(&pass_length));
    }
    m_pass_idx__ = pass_length;
    if (!buffPtrIncrementSafe(sizeof(char) * (pass_length))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(char) * (pass_length)
                   << ") Failed!";
        return false;
    }
    m_dpp_connector_length = reinterpret_cast<uint16_t *>(m_buff_ptr__);
    if (!m_parse__)
        *m_dpp_connector_length = 0;
    if (!buffPtrIncrementSafe(sizeof(uint16_t))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint16_t) << ") Failed!";
        return false;
    }
    ...
    if (m_parse__) {
        tlvf_swap(16, reinterpret_cast<uint8_t *>(&dpp_connector_length));
    }
    m_dpp_connector_idx__ = dpp_connector_length;
    if (!buffPtrIncrementSafe(sizeof(char) * (dpp_connector_length))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(char) * (dpp_connector_length)
                   << ") Failed!";
        return false;
    }
    ...
    if (m_parse__) {
        tlvf_swap(16, reinterpret_cast<uint8_t *>(&key_length));
    }
    m_ptk_idx__ = key_length;
    if (!buffPtrIncrementSafe(sizeof(uint8_t) * (key_length))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) * (key_length)
                   << ") Failed!";
        return false;
    }
    ...
    if (m_parse__) {
        tlvf_swap(16, reinterpret_cast<uint8_t *>(&group_key_length));
    }
    m_gtk_idx__ = group_key_length;
    if (!buffPtrIncrementSafe(sizeof(uint8_t) * (group_key_length))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec << sizeof(uint8_t) * (group_key_length)
                   << ") Failed!";
        return false;
    }
    ...
    if (m_parse__) {
        if (*m_type != eTlvTypeMap::TLV_VIRTUAL_BSS) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvTypeMap::TLV_VIRTUAL_BSS)
