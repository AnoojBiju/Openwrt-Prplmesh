...
VirtualBssEvent::VirtualBssEvent(uint8_t* buff, size_t buff_len, bool parse) :
    BaseClass(buff, buff_len, parse) {
    m_init_succeeded = init();
}
VirtualBssEvent::VirtualBssEvent(std::shared_ptr<BaseClass> base, bool parse) :
BaseClass(base->getBuffPtr(), base->getBuffRemainingBytes(), parse){
    m_init_succeeded = init();
}
VirtualBssEvent::~VirtualBssEvent() {
}
const eTlvTypeMap& VirtualBssEvent::type() {
    return (const eTlvTypeMap&)(*m_type);
}

const uint16_t& VirtualBssEvent::length() {
    return (const uint16_t&)(*m_length);
}

const eVirtualBssSubtype& VirtualBssEvent::subtype() {
    return (const eVirtualBssSubtype&)(*m_subtype);
}

sMacAddr& VirtualBssEvent::radio_uid() {
    return (sMacAddr&)(*m_radio_uid);
}

uint8_t& VirtualBssEvent::success() {
    return (uint8_t&)(*m_success);
}

sMacAddr& VirtualBssEvent::bssid() {
    return (sMacAddr&)(*m_bssid);
}

void VirtualBssEvent::class_swap()
{
...
size_t VirtualBssEvent::get_initial_size()
{
    size_t class_size = 0;
...
bool VirtualBssEvent::init()
{
    ...
    if (!m_parse__) *m_subtype = eVirtualBssSubtype::VIRTUAL_BSS_EVENT;
