...
tlvAssociatedWiFi6StaStatusReport::tlvAssociatedWiFi6StaStatusReport(uint8_t *buff, size_t buff_len,
                                                                     bool parse)
...
tlvAssociatedWiFi6StaStatusReport::tlvAssociatedWiFi6StaStatusReport(
    std::shared_ptr<BaseClass> base, bool parse)
...
sMacAddr &tlvAssociatedWiFi6StaStatusReport::sta_mac() { return (sMacAddr &)(*m_sta_mac); }
...
std::tuple<bool, tlvAssociatedWiFi6StaStatusReport::sTidQueueSize &>
tlvAssociatedWiFi6StaStatusReport::tid_queue_size_list(size_t idx)
{
...
bool tlvAssociatedWiFi6StaStatusReport::alloc_tid_queue_size_list(size_t count)
{
    if (m_lock_order_counter__ > 0) {
        ;
        TLVF_LOG(ERROR)
            << "Out of order allocation for variable length list tid_queue_size_list, abort!";
        return false;
    }
    size_t len = sizeof(sTidQueueSize) * count;
    LOG(DEBUG) << "****THE VALUE OF getBuffRemainingBytes()_alloc_tid_queue_size = "
               << getBuffRemainingBytes();
    LOG(DEBUG) << "****THE VALUE OF LENGTH  = " << len;
    ...
    *m_tid_queue_size_list_length += count;
    ...
    if (!m_parse__) {
        for (size_t i = m_tid_queue_size_list_idx__ - count; i < m_tid_queue_size_list_idx__; i++) {
            m_tid_queue_size_list[i].struct_init();
        }
    }
    return true;
}
...
{
    LOG(DEBUG) << "****THE VALUE OF getBuffRemainingBytes() = " << getBuffRemainingBytes();
    LOG(DEBUG) << "****THE VALUE OF get_initial_size()  = " << get_initial_size();
    ...
    if (!m_parse__)
        *m_type = eTlvTypeMap::TLV_ASSOCIATED_WIFI_6_STA_STATUS_REPORT;
    ...
    if (!buffPtrIncrementSafe(sizeof(sTidQueueSize) * (tid_queue_size_list_length))) {
        LOG(ERROR) << "buffPtrIncrementSafe(" << std::dec
                   << sizeof(sTidQueueSize) * (tid_queue_size_list_length) << ") Failed!";
        return false;
    }
    if (m_parse__) {
        class_swap();
    ...
    if (m_parse__) {
        if (*m_type != eTlvTypeMap::TLV_ASSOCIATED_WIFI_6_STA_STATUS_REPORT) {
            TLVF_LOG(ERROR) << "TLV type mismatch. Expected value: "
                            << int(eTlvTypeMap::TLV_ASSOCIATED_WIFI_6_STA_STATUS_REPORT)
