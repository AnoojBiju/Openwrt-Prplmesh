...
bool AmbiorixOdlManager::odlSaveCheckTree(amxd_object_t *object) {
...
int AmbiorixOdlManager::odlDmSaveObject(amxo_parser_t *parser, amxd_object_t *object, const char *dir, const char *name) {
    int status = 0;
    amxc_string_t file;
...
std::string AmbiorixOdlManager::odlGetDirectory() {
...
int AmbiorixOdlManager::odlDmSave(amxd_dm_t *dm, amxo_parser_t *parser) {
    int status = 0;
    amxc_var_t* paths = odlGetSaveObjects(parser);
    std::string dir = odlGetDirectory();
    ...
    LOG(INFO) << "dl directory : "<<dir.c_str();
    if(dir.empty()) {
        LOG(INFO) << "0";
    ...
    if(paths == NULL) {
        ...
        status = odlDmSaveObject(parser, root, dir.c_str(), name);
        goto exit;
    }
    LOG(INFO) << "2";
    ...
    amxc_var_for_each(path, paths) {
        ...
        status = odlDmSaveObject(parser, obj, dir.c_str(), amxc_string_get(&file_name, 0));
        index++;
    }
...
void AmbiorixOdlManager::odlSaveAddPath(const char *path) {
    //TODO : analyze this
...
amxc_var_t *AmbiorixOdlManager::odlGetSaveObjects(amxo_parser_t *parser) {
...
bool AmbiorixOdlManager::odlSaveCheckPaths(amxo_parser_t *parser, amxd_object_t *object) {
...
bool AmbiorixOdlManager::odlSaveCheckPersistentParams(amxd_object_t *object, const amxc_var_t *data) {
...
AmbiorixOdlManager::AmbiorixOdlManager():AMXOConfig(){
...
bool AmbiorixOdlManager::loadRootDM(std::string path){
...
int AmbiorixOdlManager::populateDataModel(){
    ...
    if(GETP_BOOL(AMXOParser::getConfig(), AMXOParser::OdlConfig_str(AMXOParser::CVAL_LOAD))) {
        amxc_string_setf(&include, "?include '${odl.directory}':'${odl.dm-defaults}';");
    }else{
        amxc_string_setf(&include, "include '${odl.dm-defaults}';");
    }
    status = amxo_parser_parse_string(AMXOParser::getParser(), amxc_string_get(&include, 0), root);
    LOG(INFO) << "Import - status "<<status;
    if(status != 0) {
        LOG(WARNING) << "Import - No odl directory specified or loading failure occurred";
    }

    amxc_string_t dir;
    amxc_string_init(&dir, 0);
    amxc_string_set(&dir,"include '${upc.directory};");
    amxc_string_resolve(&dir, AMXOParser::getConfig());
    auto resolved_dir = amxc_string_take_buffer(&dir);
    LOG(INFO) << "Import - resolved_dir "<<std::string(resolved_dir);

    amxc_string_setf(&include, "include '${upc.directory}';");
    status = amxo_parser_parse_string(AMXOParser::getParser(), amxc_string_get(&include, 0), root);
    LOG(INFO) << "Import - status "<<status;

    if(status != 0) {
        LOG(WARNING) << "Import - No persistent storage from /cfg/prplmesh directory";
    }
    amxp_sigmngr_enable(&AMXOParser::getDatamodel()->sigmngr, true);
...
void AmbiorixOdlManager::handleSaveRequest(const char * const sig_name, const amxc_var_t * const data) {
...
void AmbiorixOdlManager::dispatchAndSaveEvent(const char * const sig_name, const amxc_var_t * const data, void * const priv){
