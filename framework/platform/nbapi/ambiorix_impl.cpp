...
AmbiorixImpl::AmbiorixImpl(std::shared_ptr<EventLoop> event_loop, amxd_object_t *obj)
    : m_event_loop(event_loop), m_trans(obj)
{
...
bool AmbiorixImpl::apply_transaction()
{
    auto ret    = true;
    auto status = amxd_trans_apply(&(m_trans.transaction), &m_datamodel);
    if (status != amxd_status_ok) {
        LOG(ERROR) << "Couldn't apply transaction object, status: " << status;
        ret = false;
    }

    amxd_trans_clean(&(m_trans.transaction));
...
bool AmbiorixImpl::set(const std::string &relative_path, const std::string &value)
{
    ...
    if (!m_trans.prepare_transaction()) {
        return false;
    }

    LOG(DEBUG) << "Set value: " << value << "to the object: " << m_trans.object->name;

    amxd_trans_set_value(cstring_t, &(m_trans.transaction), m_trans.object->name, value.c_str());
...
bool AmbiorixImpl::set(const std::string &relative_path, const int32_t &value)
{
    ...
    amxd_trans_set_value(int32_t, &(m_trans.transaction), m_trans.object->name, value);
...
bool AmbiorixImpl::set(const std::string &relative_path, const int64_t &value)
{
    ...
    amxd_trans_set_value(int64_t, &(m_trans.transaction), m_trans.object->name, value);
...
bool AmbiorixImpl::set(const std::string &relative_path, const uint32_t &value)
{
    ...
    amxd_trans_set_value(uint32_t, &(m_trans.transaction), m_trans.object->name, value);
...
bool AmbiorixImpl::set(const std::string &relative_path, const uint64_t &value)
{
    ...
    amxd_trans_set_value(uint64_t, &(m_trans.transaction), m_trans.object->name, value);
...
bool AmbiorixImpl::set(const std::string &relative_path, const double &value)
{
    ...
    LOG(DEBUG) << "Set value: " << value << "to the object: " << m_trans.object->name;
    amxd_trans_set_value(double, &(m_trans.transaction), m_trans.object->name, value);
    if (!apply_transaction()) {
        LOG(ERROR) << "Couldn't apply transaction.";
        return false;
    }
...
bool AmbiorixImpl::set(const std::string &relative_path, const bool &value)
{
    ...
    LOG(DEBUG) << "Set value: " << value << "to the object: " << m_trans.object->name;
    amxd_trans_set_value(bool, &(m_trans.transaction), m_trans.object->name, value);
    if (!apply_transaction()) {
        LOG(ERROR) << "Couldn't apply transaction.";
        return false;
    }
...
{
    m_trans.prepare_transaction();
    if (!m_trans.object) {
        LOG(ERROR) << "Couldn't find the object for: " << relative_path;
        return false;
    }

    auto status = amxd_trans_add_inst(&(m_trans.transaction), 0, NULL);
    if (status != amxd_status_ok) {
        LOG(ERROR) << "Failed to add instance for: " << m_trans.object->name
                   << "status: " << status;
    }

    if (!apply_transaction()) {
        LOG(ERROR) << "Failed to apply transaction for: " << m_trans.object->name;
        return false;
    }

    LOG(DEBUG) << "Instance added for: " << m_trans.object->name;
    return true;
}
...
bool AmbiorixImpl::remove_instance(const std::string &relative_path, uint32_t index)
{
    ...
    amxd_object_for_each(instance, it, m_trans.object)
    {
        ...
        if (current_inst_index == index) {
            amxd_trans_del_inst(&(m_trans.transaction), amxd_object_get_index(inst), NULL);
            break;
        }
    }
    ...
    LOG(DEBUG) << "Instance removed for: " << m_trans.object->name;
    return true;
}
