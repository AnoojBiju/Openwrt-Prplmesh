...
uint32_t AmbiorixDummy::add_instance(const std::string &relative_path) { return 42; }
bool AmbiorixDummy::remove_instance(const std::string &relative_path, uint32_t index)
{
    return true;
}
int AmbiorixDummy::get_instance_index(const std::string &relative_path, const std::string &key)
{
    return 1;
}
