...
class Configuration():
    def __init__(self):
        ...
        self.test_mode_remove_out_files: bool = True
        self.test_mode_override_files: bool = True
        self.spaces_in_tab: int = 4
        self.code_snippet_open_message: str = \
            "// Enum AutoPrint generated code snippet begining- DON'T EDIT!\n"
        self.code_snippet_close_message: str = "// Enum AutoPrint generated code snippet end\n"
        self.formatter_open_str: str = ""
        self.formatter_close_str: str = ""
        self.redirect_stdout_to_log_file: bool = False
...
class EnumAutoPrint:
    ...
    def __get_enum_metadata(self, position_data: CodePosition, in_class: bool, in_header_file: bool,
    ...
    def __add_generated_code(self, enum_metadata: EnumMetadata, base_indentation: int,
                             lines_out: List[str]):

        def getIndentation(level) -> str:
            return str(" ") * (base_indentation + level * self.conf.spaces_in_tab)
        ...
        lines_out.append("{0}{1}".format(getIndentation(
            0), self.conf.code_snippet_open_message))

        if len(self.conf.formatter_open_str) > 0:
            lines_out.append("{0}{1}\n".format(getIndentation(0), self.conf.formatter_open_str))
        ...
        if len(self.conf.formatter_close_str) > 0:
            lines_out.append("{0}{1}\n".format(getIndentation(0),
                                               self.conf.formatter_close_str))

        lines_out.append("{0}{1}".format(getIndentation(
            0), self.conf.code_snippet_close_message))

    def redirect_stdout_to_file(self, output_filename: str = 'output.log'):

        path = os.path.abspath(sys.path[0] + '/' + output_filename)
        self.conf.output_file = open(path, 'w')
        sys.stdout = self.conf.output_file

    def upgrade_enum(self, file_path: str):
        ...
        for line_idx, line in enumerate(lines):
            ...
            if self.conf.code_snippet_open_message in line:
                auto_generated_snippet_to_ignore = True
            elif self.conf.code_snippet_close_message in line:
                ...
                if len(positions_stack) > 0 and (char == "{" or joker_char == "{"):
                    ...
                    if positions_stack[-1].curl_bracket_balance == 0:
                        if ("enum" in positions_stack[-1].position_of_type):
                            ...
                            in_class: bool = len(
                                positions_stack) > 1 and \
                                positions_stack[-2].position_of_type == "class"
                            enum_metadata = self.__get_enum_metadata(
                            ...
                            if skip_next_enum:
                                ...
                                self.__add_generated_code(enum_metadata, indentation, lines_out)

                        # enum/class/struct block is closed and can be removed from the stack.
                        positions_stack.pop()

        # Write output file
        if ((not self.conf.test_mode) or
                (self.conf.test_mode and self.conf.test_mode_override_files)):
            f = open(file_path, "w")
        else:
...
def get_files_list(conf: Configuration, workspace_root: str, exclude_files_paths: str) -> list:
    ...
    for subdir, dirs, files in os.walk(workspace_root):
        ...
        for file in files:
            ...
            if conf.test_mode_remove_out_files and file.endswith(".out"):
                os.remove(file)
                continue
...
def read_configuration(conf: Configuration) -> Tuple[str, str, str]:
    std_out_str = io.StringIO()
    old_stdout = sys.stdout
    sys.stdout = std_out_str
    ...
    if (redirect_stdout_to_log_file_key in configuration.keys()):
        if configuration[redirect_stdout_to_log_file_key]:
            conf.redirect_stdout_to_log_file = configuration[redirect_stdout_to_log_file_key]
    ...
    if (test_mode_remove_out_files_key in configuration.keys()):
        conf.test_mode_remove_out_files = configuration[test_mode_remove_out_files_key]
    print("test_mode_remove_out_files:", conf.test_mode_remove_out_files)

    test_mode_override_files_key = 'test_mode_override_files'
    if (test_mode_override_files_key in configuration.keys()):
        conf.test_mode_override_files = configuration[test_mode_override_files_key]
    print("test_mode_override_files:", conf.test_mode_override_files)

    spaces_in_tab_key = 'spaces_in_tab'
    if (spaces_in_tab_key in configuration.keys()):
        conf.spaces_in_tab = configuration[spaces_in_tab_key]

    formatter_open_direction_key = 'formatter_open_direction'
    if (formatter_open_direction_key in configuration.keys()):
        conf.formatter_open_str = configuration[formatter_open_direction_key]
    ...
    if test_workspace_root_key not in configuration.keys() and conf.test_mode:
        raise Exception('Bad configuration: Missing field "{}" on configuration file'.format(
    ...
    return workspace_root, exclude_files_paths, std_out_str
...
if __name__ == '__main__':
    ...
    conf.test_mode = args.test

    # Read configuration file. Get the workspace root folder and an excluded files list
    workspace_root, exclude_files_paths, stdout_prints = read_configuration(conf)

    eap = EnumAutoPrint()
    eap.conf = conf

    if eap.conf.redirect_stdout_to_log_file:
        eap.redirect_stdout_to_file()

    # Print stdout print of read_configuration() function
    print(stdout_prints)

    # Get files list.
    files_list = get_files_list(conf, workspace_root, exclude_files_paths)
    print("Files List:", *files_list, sep='\n', end="\n\n")

    for file_path in files_list:
        eap.upgrade_enum(file_path)
