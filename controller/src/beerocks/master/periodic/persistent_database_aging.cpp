...
void persistent_database_aging_operation::periodic_operation_function()
{
    ...
    std::copy_if(
        ...
        [&](const sMacAddr &client_mac) {
            ...
            auto temp_timelife =
                std::chrono::seconds(m_database.get_client_time_life_delay(client_mac));
            ...
            if ((temp_timelife == std::chrono::seconds::zero())) {
                OPERATION_LOG(TRACE) << "danie 0";
                return false;
            } else if (temp_timelife > std::chrono::seconds::zero()) {

                expiry_due = parameters_last_edit + temp_timelife;
            }
            OPERATION_LOG(TRACE) << "danie 1"
                                 << "temp" << temp_timelife.count() << "last_edit"
                                 << parameters_last_edit.time_since_epoch().count() << "last_aging"
                                 << last_aging_check.time_since_epoch().count() << "expiry_due"
                                 << expiry_due.time_since_epoch().count() << "diff expiry_due"
                                 << expiry_due.time_since_epoch().count() -
                                        last_aging_check.time_since_epoch().count();
    ...
    for (const auto &aged_client : aged_clients) {
        OPERATION_LOG(TRACE) << "danie 5";

        m_database.clear_client_persistent_db(aged_client);
    }
    OPERATION_LOG(TRACE) << "danie 4";
}
