                                                      ...
                                                      ieee1905_1::CmduMessageRx &cmdu_rx)
{
    ...
    if (database.is_prplmesh(src_mac)) {
        /**
         * For prplmesh agents, we need to check if there an additional
         * VS TLV with extended scan result information.
         */
        auto beerocks_header = beerocks::message_com::parse_intel_vs_message(cmdu_rx);
        if (!beerocks_header) {
        ...
        for (auto const vs_result_tlv :
            ...
            database.update_channel_report(vs_result_tlv->radio_uid(),
                                           vs_result_tlv->operating_class(),
