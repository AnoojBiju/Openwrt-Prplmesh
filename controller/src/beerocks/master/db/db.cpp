                              ...
                              const beerocks::message::sRadioCapabilities &sta_cap)
{
    ...
    if (!pSta) {
        LOG(ERROR) << "Station not found " << client_mac;
        return false;
    }

    auto parent_radio = pSta->get_bss();
    if (!parent_radio) {
        LOG(ERROR) << "Parent radio not found for client: " << client_mac;
        return false;
    }

    const std::string &bssid = tlvf::mac_to_string(parent_radio->bssid);
    if (bssid == network_utils::ZERO_MAC_STRING) {
        LOG(ERROR) << "parent radio found for client " << client_mac << " is empty";
        return false;
    }

    if (is_node_5ghz(bssid)) {
