...
{
    auto n = get_slave_node(hostap_mac);
    if (!n) {
    ...
    const sMacAddr &radio_mac, wfa_map::cRadiosWithScanCapabilities &radio_capabilities)
{
    ...
    auto node = get_slave_node(radio_mac);
    if (!node) {
...
{
    std::shared_ptr<node_slave> n = get_slave_node(mac);
...
std::deque<sMacAddr> db::get_clients_with_persistent_data_configured()
{
    ...
    for (auto node_map : nodes) {
        for (auto kv : node_map) {
            if (kv.second->get_type() == eType::TYPE_CLIENT) {
                auto client = std::dynamic_pointer_cast<node_client>(kv.second);

                if ((client->mac == kv.first) &&
                    (client->client_parameters_last_edit !=
...
std::shared_ptr<node_client> db::get_client_node(const sMacAddr &mac) {
    auto node = get_node_verify_type(mac, beerocks::TYPE_CLIENT);
    if (!node) {
        return nullptr;
    }
    return std::dynamic_pointer_cast<node_client>(node);
}

std::shared_ptr<node_slave::radio> db::get_hostap_by_mac(const sMacAddr &mac)
{
    auto n = get_slave_node(mac);
    if (!n) {
        LOG(ERROR) << "node not found.... ";
...
sMacAddr db::get_candidate_client_for_removal(sMacAddr client_to_skip)
{
    ...
    for (const auto &node_map : nodes) {
        for (const auto &key_value : node_map) {
            if (key_value.second->get_type() != beerocks::eType::TYPE_CLIENT) {
                continue;
            }
            const auto client = std::dynamic_pointer_cast<node_client>(key_value.second);
