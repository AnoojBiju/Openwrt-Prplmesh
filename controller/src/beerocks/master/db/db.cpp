...
bool db::load_persistent_db_clients()
{
    ...
    // better usage with vectors, reserving space beforehand
    vector_of_clients.reserve(config.clients_persistent_db_max_size *
                              std::extent<decltype(vector_of_clients)>::value);
    ...
    // it'll will be removed aswell as not getting further down this function pipeline.
    std::copy_if(
        clients.begin(), clients.end(), vector_of_clients.begin(),
        [&](const std::pair<std::string, std::unordered_map<std::string, std::string>> &client_pair)
            -> bool {
            ...
            if (client_mac != network_utils::ZERO_MAC) {
                ...
                remove_client_entry_and_update_counter(client_pair.first);
                return false;
            }
