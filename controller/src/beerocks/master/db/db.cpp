...
bool db::load_persistent_db_clients()
{
    ...
    // Print counters
    LOG_IF(diff, DEBUG) << diff << "were deleted because they hit the threshold";
    LOG_IF(diff, DEBUG) << set_error_mac_count << "were deleted because of malformed mac address";
    LOG_IF(add_error_count, DEBUG) << "Failed to add nodes for " << add_error_count << " clients";
    LOG_IF(set_error_count, DEBUG) << "Failed to set nodes with values from persistent db for "
