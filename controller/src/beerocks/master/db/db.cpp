...
bool db::load_persistent_db_clients()
{
    ...
    LOG_IF(add_error_count, DEBUG) << "Couldn't add to nodes for " << add_error_count << "clients";
    LOG_IF(set_error_count, DEBUG) << "Couldn't set the nodes with values from persistent db for "
                                   << set_error_count << " clients";
    LOG(DEBUG) << "Filtered clients: " << diff << " due to max DB capacity reached:"
