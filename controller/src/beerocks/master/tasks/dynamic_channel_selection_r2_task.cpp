    ...
    const sScanReportEvent &scan_report_event)
{
    ...
    auto has_matching_report_index =
        ...
        for (const auto &stored_report_key : stored_report_index) {
            if (request_scan_report_copy.erase(stored_report_key) != 1) {
                LOG(TRACE) << "Request has missing stored report keys: " << stored_report_key.first
                           << "-" << stored_report_key.second;
                return false;
            }
        }
        if (!request_scan_report_copy.empty()) {
            LOG(TRACE) << "Stored report does not contain all requested record keys";
            for (const auto &request_report_key : request_scan_report_copy) {
                LOG(TRACE) << request_report_key.first << "-" << request_report_key.second;
            }
            return false;
        }
        return true;
    ...
    // No need to print an error since it would have been printed beforehand.
    if (!scan_req_ptr) {
        return false;
    }
     ...
     * instead set the interval for it's next iteration.
     */
    if (!scan_req_ptr->is_single_scan) {
        if (database.get_channel_scan_is_enabled(radio_mac)) {
            LOG(TRACE) << "Continuous scans on radio " << radio_mac << " are still enabled";
            ...
            LOG(TRACE) << "Removing continuous scan for radio " << radio_mac << " from agent";
            agent_status.continuous_radio_scans.erase(radio_mac);
        }

    } else {
        LOG(TRACE) << "Removing single scan for radio " << radio_mac << " from agent";
