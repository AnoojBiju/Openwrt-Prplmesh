...
void cli_bml::setFunctionsMapAndArray()
{
    ...
    insertCommandToMap("bml_set_dynamic_channel_pool", "<mac> <channel pool>",
                       "Set the dynamic channel pool for the Auto Channel Selection. channels are "
                       "separated by commas.",
                       static_cast<pFunction>(&cli_bml::bml_set_dynamic_channel_pool_caller), 2, 2,
                       STRING_ARG, STRING_ARG);
...
int cli_bml::bml_set_dynamic_channel_pool_caller(int numOfArgs)
{
    if (numOfArgs == 2) {
        return set_dynamic_pool(args.stringArgs[0], args.stringArgs[1]);
    }
    return -1;
}
...
{
    int ret =
        bml_on_demand_channel_selection(ctx, radio_mac.c_str(), channel, bandwidth, csa_count);
    printBmlReturnVals("channel_selection", ret);
    return 0;
}

int cli_bml::set_dynamic_pool(const std::string &radio_mac, const std::string &channel_pool)
{
    ...
    int ret = bml_set_dynamic_pool(ctx, radio_mac.c_str(), channel_pool_arr, channels_size);
    printBmlReturnVals("set_dynamic_pool", ret);
    return 0;
}
