...
* Ex. if we have string "0 1 5 63" then 1st, 2d, 5th and 64th bit will be set to 1.
...
static uint64_t get_uint64_from_bss_color_bitmap(const std::string decimal_str)
{
    ...
    while (iss >> value) {
        decimal_values.push_back(value);
    }

    for (const int &decimal : decimal_values) {
        // Ensure that the decimal value is within the valid range (0-63)
        if (decimal >= 0 && decimal <= 63) {
            // Set corresponding bits at result variable to 1.
            uint64_t bitmask = 1ULL << decimal;
            result |= bitmask;
        } else {
